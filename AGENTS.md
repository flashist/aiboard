...
2. **Take it.** `aiboard task assign T-007 my-agent-name` then `aiboard task change-status T-007 in-progress`
...
5. **Finish.** `aiboard task change-status T-007 done --note "PR #12 merged"` or `aiboard task change-status T-007 cancelled --note "superseded by T-009"`.
